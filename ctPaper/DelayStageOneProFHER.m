function [ matout ] = DelayStageOneProFHER( basic, advanced, mat )
...
%advanced.DOPLOT = 0 ; 
    ...
    if advanced.nochangeallowed % In stage I, no change allowed implies that stopping now before new data arrives
        ...
        bestsvec(beststage1 == max(muvec'*PPMax-ICost,0)) = 0;
                ...
                %MF 9/12/19 ADJUST TO ACCOUNT FOR FIXED COST OF TRIAL AND
                %TRANSFORMATION
            %basic.addon =  - basic.P * basic.pN * muvec' - basic.ICostS ;  % adjusted add-on at end of trial
            basic.addon = 0 ;  % adjusted add-on at end of trial
            valStage1 = beststage1 - basic.fixedCost + basic.addon ;
            valNoTrial = muvec'*PPMax ;
            beststage1 = max( valStage1, max( valNoTrial,0) + basic.addon );      % check if s=0 is even better than positive s in (0, tau]
            bestsvec(beststage1 == max(valNoTrial ,0)+ basic.addon) = 0;
    end

    % MF 9/12/19 Adjust B0hat to account for the untransformed reward
    B0hat(:) = B0hat(:) - basic.fixedCost + basic.addon ;
    %plot(muvec,B0hat,'-',muvec,beststage1','-.',muvec,max(muvec*PPatientvec(1)-ICost,0),'--');
    %xlim( [ -10000,10000] ) ;
    %legend('value of continuing to phase II','optimal stage I value','stop now','value of continuing to phase II-FC','optimal stage I value-FC','stop now-FC');
