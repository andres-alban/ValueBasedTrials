...
% 1. 41,000 (out of 138,000) samples find no trial optimal
% 2. 40,000 (out of 138,000) samples find r = rmax optimal
% 3. 209 (out of 138,000) samples find T = Tmax optimal
% 4. in total 56,000 (out of 138,000) are in the interior (~40.5%)
% 5. It takes around 4h to run this script
...
for i = 1:combs
    ...
    pN = results(i,9);
    basic.pN = pN;
    [Design,ENGstar] = OneShotMaxExpectedNetGain(basic,true,'Tr',10,false);
    results(i,12) = ENGstar;
    results(i,13) = Design(1);
    results(i,14) = Design(2);
    results(i,15) = Design(1)*Design(2)/2;
    if mod(i,1e3) == 0
        fprintf('%d/%d\n',i,combs);
    end
end
...
save('explore_caseIV.mat','results','T')
