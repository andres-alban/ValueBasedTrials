% This script takes ~33''
load('explore_caseIII.mat')
...
c = 1000;
sigmaX = 1000;
results_pos = results(results(:,15)~=0 & results(:,14)<basic.rmax-0.01,:);
...
for i = 1:size(results_pos,1)
    ...
    % PH
    if results_pos(i,1)
        [dENG,dT,dr,dQ,Vrr] = OneShotCompStats_caseIII(basic,T,r,'Population',1,0.01);
    else
        [dENG,dT,dr,dQ,Vrr] = OneShotCompStats_caseIII(basic,T,r,'H',0.1,0.01);
    end
    comp_stats_PH(i,1) = dENG; comp_stats_PH(i,2) = dT; comp_stats_PH(i,3) = dr; comp_stats_PH(i,4) = dQ; comp_stats_PH(i,5) = Vrr;
    % n0
    [dENG,dT,dr,dQ,Vrr] = OneShotCompStats_caseIII(basic,T,r,'n0',1e-3,0.01);
    comp_stats_n0(i,1) = dENG; comp_stats_n0(i,2) = dT; comp_stats_n0(i,3) = dr; comp_stats_n0(i,4) = dQ; comp_stats_n0(i,5) = Vrr;
    % sigmaX
    [dENG,dT,dr,dQ,Vrr] = OneShotCompStats_caseIII(basic,T,r,'sigmaX',1,0.01);
    comp_stats_sigmaX(i,1) = dENG; comp_stats_sigmaX(i,2) = dT; comp_stats_sigmaX(i,3) = dr; comp_stats_sigmaX(i,4) = dQ; comp_stats_sigmaX(i,5) = Vrr;
    % c
    [dENG,dT,dr,dQ,Vrr] = OneShotCompStats_caseIII(basic,T,r,'c',1,0.01);
    comp_stats_c(i,1) = dENG; comp_stats_c(i,2) = dT; comp_stats_c(i,3) = dr; comp_stats_c(i,4) = dQ; comp_stats_c(i,5) = Vrr;
    % cr
    [dENG,dT,dr,dQ,Vrr] = OneShotCompStats_caseIII_cr(basic,T,r,cr,theta,0.1,0.01);
    comp_stats_cr(i,1) = dENG; comp_stats_cr(i,2) = dT; comp_stats_cr(i,3) = dr; comp_stats_cr(i,4) = dQ; comp_stats_cr(i,5) = Vrr;
...
% Save data
save('SimCompStats_caseIII.mat','results_pos','comp_stats_PH','comp_stats_n0','comp_stats_sigmaX','comp_stats_c','comp_stats_cr')

% print results
fileID = fopen('comp_stats_caseIII.txt','w');
